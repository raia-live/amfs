...
def _record(outcome: OutcomeType, keys: list[str], attempts: list[AttemptRecord] | None = None) -> OutcomeRecord:
...
class TestSingleOutcome:
    ...
    def test_success_never_sets_discredited(self) -> None:
        ...
        # re-stamp it either.
        e = _entry(0.2, evidence_failure=6.0, prior_confidence=0.7, failure_count=3,
                   discredited_at=datetime.now(timezone.utc))
...
class TestRegimeChangeTimescale:
    ...
    def _validated(self) -> MemoryEntry:
        ...
        for _ in range(10):
            upd = ev.apply_outcome(e, OutcomeType.SUCCESS)
            e = e.model_copy(update=upd.as_entry_update(datetime.now(timezone.utc)))
    ...
    def test_one_failure_contests_two_discredit(self) -> None:
        ...
        e = e.model_copy(update=first.as_entry_update(datetime.now(timezone.utc)))
    ...
    def test_recovery_clears_discredit(self) -> None:
        e = _entry(0.7)
        e = e.model_copy(update=ev.apply_outcome(e, OutcomeType.FAILURE).as_entry_update(datetime.now(timezone.utc)))
        assert e.discredited_at is not None
        for _ in range(3):
            e = e.model_copy(update=ev.apply_outcome(e, OutcomeType.SUCCESS).as_entry_update(datetime.now(timezone.utc)))
...
class TestRecordApplication:
    def test_attempt_failure_then_success_splits_credit(self) -> None:
        ...
        assert new_stale.confidence < ev.DISCREDIT_THRESHOLD and new_stale.discredited_at is not None
    ...
    def test_multiplicative_flag_restores_legacy(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        new, _ = ev.apply_record_to_entry(_entry(0.9), _record(OutcomeType.FAILURE, ["svc/mod/rule"]))
...
class TestContrastLesson:
    def test_fail_then_succeed_produces_lesson(self) -> None:
        record = _record(
            ...
            attempts=[AttemptRecord(attempt=1, causal_entry_keys=["svc/mod/stale"], summary="tried restart")],
        )
    ...
    def test_no_lesson_without_failed_attempts_or_on_failure(self) -> None:
        ...
        rec = _record(OutcomeType.FAILURE, ["a/b/c"], attempts=[AttemptRecord(attempt=1, causal_entry_keys=["a/b/d"])])
...
class TestStatus:
    def test_vocabulary(self) -> None:
        ...
        assert _entry(0.3, failure_count=1, discredited_at=datetime.now(timezone.utc)).evidence_status == "discredited"
