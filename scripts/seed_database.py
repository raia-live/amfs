...
def seed_immutable_traces(cur):
    """Create amfs_immutable_traces table if missing and seed with Pro trace data."""
    print("  Seeding immutable traces (Pro)...")

    cur.execute("""
        CREATE TABLE IF NOT EXISTS amfs_immutable_traces (
            ...
            causal_entries JSONB NOT NULL DEFAULT '[]',
            write_events JSONB NOT NULL DEFAULT '[]',
            tool_calls JSONB NOT NULL DEFAULT '[]',
            external_contexts JSONB NOT NULL DEFAULT '[]',
            agent_interactions JSONB NOT NULL DEFAULT '[]',
            ...
            signing_key_id TEXT NOT NULL DEFAULT 'hmac-v1',
            schema_version INTEGER NOT NULL DEFAULT 1,
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
            CONSTRAINT uq_session_sequence UNIQUE (namespace, session_id, sequence_number)
        )
    """)
    ...
    def make_sig(content_hash: str) -> str:
        return hmac.new(signing_key.encode(), content_hash.encode(), hashlib.sha256).hexdigest()

    imm_traces = []
    parent_hash = None

    for i, tid in enumerate(TRACE_IDS[:6]):
        agent = ["deploy-agent", "review-agent", "security-scanner", "monitoring-agent", "deploy-agent", "perf-optimizer"][i]
        session = SESSIONS[agent]
        outcome_refs = ["DEP-100", "REV-200", "SEC-300", "MON-400", "DEP-101", "OPT-500"]
        outcome_types = ["clean_deploy", "regression", "p2_incident", "p1_incident", "clean_deploy", "clean_deploy"]
        summaries = [
            "Deployed checkout-service v2.14 with updated retry config",
            "Auth session timeout regression — users logged out prematurely",
            "Token leak vulnerability detected and JWT rotation initiated",
            "P1: Double charge incident in payments service",
            "Circuit breaker config deployed with canary validation",
            "Cache and DB pool optimization — 63% latency improvement",
        ]

        llm_calls = [
            [
                {"call_id": uid(), "model": "gpt-4o", "provider": "openai",
                 "prompt_tokens": 1200, "completion_tokens": 450, "total_tokens": 1650,
                 "temperature": 0.1, "max_tokens": 2000, "latency_ms": 2340,
                 "cost_usd": 0.0285, "finish_reason": "stop", "started_at": ts_iso(12.8)},
                {"call_id": uid(), "model": "gpt-4o-mini", "provider": "openai",
                 "prompt_tokens": 800, "completion_tokens": 200, "total_tokens": 1000,
                 "temperature": 0.0, "max_tokens": 1000, "latency_ms": 890,
                 "cost_usd": 0.0045, "finish_reason": "stop", "started_at": ts_iso(12.5)},
            ],
            [
                {"call_id": uid(), "model": "claude-sonnet-4-20250514", "provider": "anthropic",
                 "prompt_tokens": 2100, "completion_tokens": 890, "total_tokens": 2990,
                 "temperature": 0.2, "max_tokens": 4000, "latency_ms": 3200,
                 "cost_usd": 0.042, "finish_reason": "stop", "started_at": ts_iso(37)},
            ],
            [
                {"call_id": uid(), "model": "gpt-4o", "provider": "openai",
                 "prompt_tokens": 3500, "completion_tokens": 1200, "total_tokens": 4700,
                 "temperature": 0.0, "max_tokens": 4000, "latency_ms": 4100,
                 "cost_usd": 0.072, "finish_reason": "stop", "started_at": ts_iso(81)},
                {"call_id": uid(), "model": "gpt-4o", "provider": "openai",
                 "prompt_tokens": 1800, "completion_tokens": 600, "total_tokens": 2400,
                 "temperature": 0.1, "max_tokens": 2000, "latency_ms": 2800,
                 "cost_usd": 0.039, "finish_reason": "stop", "started_at": ts_iso(80.5)},
            ],
            [
                {"call_id": uid(), "model": "claude-sonnet-4-20250514", "provider": "anthropic",
                 "prompt_tokens": 4200, "completion_tokens": 1800, "total_tokens": 6000,
                 "temperature": 0.1, "max_tokens": 8000, "latency_ms": 5600,
                 "cost_usd": 0.084, "finish_reason": "stop", "started_at": ts_iso(49)},
                {"call_id": uid(), "model": "gpt-4o-mini", "provider": "openai",
                 "prompt_tokens": 600, "completion_tokens": 150, "total_tokens": 750,
                 "temperature": 0.0, "max_tokens": 500, "latency_ms": 650,
                 "cost_usd": 0.003, "finish_reason": "stop", "started_at": ts_iso(48.5)},
            ],
            [
                {"call_id": uid(), "model": "gpt-4o-mini", "provider": "openai",
                 "prompt_tokens": 900, "completion_tokens": 300, "total_tokens": 1200,
                 "temperature": 0.0, "max_tokens": 1000, "latency_ms": 780,
                 "cost_usd": 0.005, "finish_reason": "stop", "started_at": ts_iso(6.5)},
            ],
            [
                {"call_id": uid(), "model": "gpt-4o", "provider": "openai",
                 "prompt_tokens": 2000, "completion_tokens": 700, "total_tokens": 2700,
                 "temperature": 0.1, "max_tokens": 3000, "latency_ms": 3100,
                 "cost_usd": 0.041, "finish_reason": "stop", "started_at": ts_iso(19)},
            ],
        ]
        ...
        calls = llm_calls[i]
        total_tokens_val = sum(c.get("total_tokens", 0) for c in calls)
        total_cost_val = sum(c.get("cost_usd", 0) for c in calls)

        content = json.dumps({"outcome": outcome_refs[i], "summary": summaries[i]})
        content_hash = make_hash(content)
        sig = make_sig(content_hash)
        snapshot_hash = make_hash(f"snapshot-{i}")

        imm_traces.append((
            NS, None, agent, session, i + 1,
            outcome_refs[i], outcome_types[i], f"decision-{outcome_refs[i].lower()}",
            summaries[i],
            json.dumps([]),  # causal_entries
            json.dumps(write_events_data[i], default=str),
            json.dumps(tool_calls_data[i]),
            json.dumps([]),  # external_contexts
            json.dumps([]),  # agent_interactions
            json.dumps(calls, default=str),
            len(calls), total_tokens_val, round(total_cost_val, 4),
            snapshot_hash, content_hash, parent_hash, sig, "hmac-v1", 1,
            ts_iso(12 * (i + 1)),
        ))
...
def seed_extracted_entities(cur):
    """Create extraction tables if missing and seed with entity/relationship data."""
    print("  Seeding extracted entities & relationships...")

    cur.execute("""
        CREATE TABLE IF NOT EXISTS amfs_extracted_entities (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            namespace TEXT NOT NULL DEFAULT 'default',
            name TEXT NOT NULL,
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_entities_name ON amfs_extracted_entities (namespace, name)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_entities_type ON amfs_extracted_entities (namespace, entity_type)")

    cur.execute("""
        CREATE TABLE IF NOT EXISTS amfs_extracted_relationships (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
            namespace TEXT NOT NULL DEFAULT 'default',
    ...
    cur.execute("DELETE FROM amfs_extracted_entities WHERE namespace = %s", (NS,))
    cur.execute("DELETE FROM amfs_extracted_relationships WHERE namespace = %s", (NS,))

    entities = [
        (NS, "checkout-service", "service", json.dumps({"language": "Python", "framework": "FastAPI", "team": "product-backend"}),
         TRACE_IDS[0], "checkout-service/retry-pattern", 0.95, ts_iso(12)),
        (NS, "auth", "service", json.dumps({"language": "Go", "framework": "gin", "team": "security-compliance"}),
         TRACE_IDS[2], "auth/jwt-rotation", 0.92, ts_iso(80)),
        (NS, "payments", "service", json.dumps({"language": "Python", "provider": "Stripe", "team": "product-backend"}),
         TRACE_IDS[3], "payments/stripe-config", 0.94, ts_iso(48)),
        (NS, "Stripe", "external_service", json.dumps({"type": "payment_processor", "api_version": "2024-12-18"}),
         TRACE_IDS[3], None, 0.98, ts_iso(48)),
        (NS, "PagerDuty", "tool", json.dumps({"type": "incident_management", "integration": "API"}),
         TRACE_IDS[0], None, 0.90, ts_iso(12)),
        (NS, "Redis", "infrastructure", json.dumps({"use_case": "caching", "version": "7.x"}),
         TRACE_IDS[3], "payments/idempotency-pattern", 0.88, ts_iso(48)),
        (NS, "SendGrid", "external_service", json.dumps({"type": "email_provider", "daily_limit": 100000}),
         TRACE_IDS[6], "notification-service/email-provider", 0.91, ts_iso(60)),
        (NS, "deploy-agent", "agent", json.dumps({"type": "deployment", "deployments": 3}),
         None, None, 0.99, ts_iso(2)),
        (NS, "security-scanner", "agent", json.dumps({"type": "security", "scans": 12}),
         None, None, 0.99, ts_iso(80)),
        (NS, "monitoring-agent", "agent", json.dumps({"type": "observability", "alerts": 5}),
         None, None, 0.99, ts_iso(48)),
        (NS, "Sarah Chen", "person", json.dumps({"role": "Platform Lead", "team": "platform-engineering"}),
         None, None, 0.85, ts_iso(100)),
        (NS, "user-service", "service", json.dumps({"language": "TypeScript", "framework": "NestJS"}),
         TRACE_IDS[5], "user-service/cache-strategy", 0.90, ts_iso(18)),
    ]

    for e in entities:
        cur.execute("""
            INSERT INTO amfs_extracted_entities
                (namespace, name, entity_type, attributes, source_trace_id, source_entry_key, confidence, extracted_at)
            VALUES (%s, %s, %s, %s::jsonb, %s, %s, %s, %s)
        """, e)

    relationships = [
        (NS, "checkout-service", "Stripe", "depends_on", "Checkout uses Stripe for payment processing", ts_iso(200), None, True, TRACE_IDS[3], 0.95, ts_iso(48)),
        (NS, "checkout-service", "Redis", "uses", "Cart data cached in Redis", ts_iso(150), None, True, TRACE_IDS[3], 0.88, ts_iso(48)),
        (NS, "payments", "Stripe", "depends_on", "Core payment processing dependency", ts_iso(300), None, True, TRACE_IDS[3], 0.98, ts_iso(48)),
        (NS, "payments", "Redis", "uses", "Idempotency keys stored in Redis", ts_iso(168), None, True, TRACE_IDS[3], 0.92, ts_iso(48)),
        (NS, "auth", "Redis", "uses", "Session storage in Redis", ts_iso(200), None, True, TRACE_IDS[2], 0.85, ts_iso(80)),
        (NS, "notification-service", "SendGrid", "depends_on", "Email delivery via SendGrid", ts_iso(250), None, True, TRACE_IDS[6], 0.94, ts_iso(60)),
        (NS, "deploy-agent", "checkout-service", "manages", "Deploys and configures checkout-service", ts_iso(100), None, True, TRACE_IDS[0], 0.92, ts_iso(12)),
        (NS, "deploy-agent", "PagerDuty", "reads_from", "Checks PagerDuty before deployments", ts_iso(100), None, True, TRACE_IDS[0], 0.90, ts_iso(12)),
        (NS, "security-scanner", "auth", "manages", "Scans and configures auth security", ts_iso(120), None, True, TRACE_IDS[2], 0.93, ts_iso(80)),
        (NS, "monitoring-agent", "payments", "monitors", "Watches payments for anomalies", ts_iso(100), None, True, TRACE_IDS[3], 0.91, ts_iso(48)),
        (NS, "Sarah Chen", "checkout-service", "owns", "Platform lead responsible for checkout", ts_iso(200), None, True, None, 0.80, ts_iso(100)),
        (NS, "user-service", "Redis", "uses", "Write-through cache for user profiles", ts_iso(150), None, True, TRACE_IDS[5], 0.87, ts_iso(18)),
    ]

    for r in relationships:
        cur.execute("""
            INSERT INTO amfs_extracted_relationships
                (namespace, source_entity, target_entity, relationship_type, description,
                 valid_from, valid_until, is_current, source_trace_id, confidence, extracted_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, r)

    print(f"    Inserted {len(entities)} entities, {len(relationships)} relationships")
...
def main():
    ...
    with psycopg.connect(DSN) as conn:
        with conn.cursor() as cur:
            ...
            seed_api_keys(cur)
            seed_audit_log(cur)
            seed_immutable_traces(cur)
            seed_extracted_entities(cur)
    ...
    print("  -  6 immutable traces with LLM call spans and crypto fields")
    print("  - 12 extracted entities and 12 relationships")
