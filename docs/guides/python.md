...
**Follow the plan, not just the first action.** `guidance.plan` is the order to try actions in for the whole budget: the recommendation's action first, then what has a winning record on tasks like this, then the untried candidates — never an action that failed here. `suggested_action` is one attempt's worth of advice; an agent with three attempts that follows it and fails is otherwise back on its own instinct, which on tasks like this has already failed. The rendered text orders only what the record can stand behind (`Try X first, then Y.`), names what not to spend an attempt on, and lists the untried candidates as a choice for the agent's own judgement — their order in `plan` is a rotation by agent name so a fleet spreads out, not knowledge about the task, and an `explore`'s `suggested_action` is that same assignment. An agent with judgement of its own (any LLM) should use it over the pick; a scripted agent takes the plan as sent.

**Declare the situation, and the record is the situation's own.** With `begin(..., situation=...)` the outcome is stored under that label and, on the next run that declares the same one, the priors are counted over *that situation's* outcomes: the neighbourhood's other situations are shown apart, per label, as "on nearby kinds of task". Two situations that differ by one word embed within a hair of each other — a jest snapshot failure on a UI PR and on a backend PR read alike, and take opposite fixes — so text alone pools them; the label is what tells them apart. The situation's record is read by label, not by neighbourhood: a class whose tickets embed far from every other class's still gets its own record, and a situation with no outcomes yet gets `abstain` and no plan rather than the neighbour's — or the whole entity's — winner. One win on the situation's own record is acted on (`won 1/1 on this exact situation`): the same task solved is not the luck a pooled 1/1 might be. On that record a validated action that loses once on a task nothing then solved is read as having turned: the mode is `explore`, the action is kept second in the plan as the hedge. And once a task on the situation has ended unsolved, an `explore` there is a *sweep* (`recommendation.sweep`): the agent's judgement has been tried on this exact situation and failed, so the order over the untried is the plan — `Try A first, then B -> C` — rather than a list to choose from; on a pooled record the explore's pick stays an assignment the agent may overrule. Use a stable class label as the situation (the triage category, the failing check plus the kind of change), never the task text: a label that varies per task never matches and gets you the pooled record. Runs that declare nothing see the pooled behaviour, unchanged.

**Lessons for the exact situation outrank the pooled record.** Priors are counted over the outcomes nearest the task, and that neighbourhood can hold two classes of task with opposite answers — a dependency audit on a pinned package and one on an unpinned package read alike, and `bump_dependency` wins on one while a house rule forbids it on the other. A lesson written with `run.learn(situation, action, worked)` is about one class. When a shown lesson's situation is the one the run declared (`begin(..., situation=...)`), or its words are all in the task text, the plan is re-ordered by it: an action a lesson says did not work here goes to the back, one it says worked goes to the front (after the recommendation's own action). `guidance.applicable_lessons` lists the claims that were read as about this task. A declared situation is also stamped on the trace (attribute `situation`), which is how the repair loop tells runs on the same kind of task from runs that merely share the agent's tools. The server does the same re-ordering when it computes the plan, so an MCP agent reads the same order.
