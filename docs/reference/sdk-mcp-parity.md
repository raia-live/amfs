...
| Attributes (`customer`, `task_type`, `environment`, ...) | **By the developer** | `trace_session(mem, attributes={...})`, `mem.set_session_attributes({...})`, `mem.clear_session_attributes()`, `mem.commit_outcome(..., attributes={...})`; TS: `memory.setSessionAttributes({...})`, `memory.clearSessionAttributes()` or `commitOutcomeAsync(..., { attributes })`. Scalars only, at most 20 keys of up to 64 characters, string values up to 256 characters; keys are lowercased. `source`, `model`, `client` and `outcome_type` are reserved and stamped by the server; `memory_branch` is stamped by the SDK off `main` and does not count against the cap. |
